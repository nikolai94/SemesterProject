...
public class GenericResource {
    ...
    public String getFlightDates(@PathParam("startAirport") String startAirport,
        ...
        if (facade.afbudsrejser(startAirport, datoSorter(guf.toGMTString())).size() == 0) {
            ErrorCode error = new ErrorCode(1, "No available flights", "Flight was not found");
    ...
    public String getStartEndAirport(@PathParam("startAirport") String startAirport,
        ...
        if (facade.getFlightsWithAirportsAndDate(startAirport, slutAirport, datoSorter(guf.toGMTString())).size() == 0)
        {
            ErrorCode error = new ErrorCode(1, "No available flights", "Flight was not found");
            return gson.toJson(error);
        } 
